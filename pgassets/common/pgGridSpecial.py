    ...
    def __init__(self, pos, size, shape, objects, color=(0, 0, 0), borderwidth=0, transparent=True, bordercolor=(0, 0, 0)):
        ...
        for j in range(self.shape[0]):
            for i in range(self.shape[1]):
                ...
                self.objects[index].set_pos((int(self.rect.left + h_offset + j*gap_x + i*self.objects[index].rect.width),
                                            int(self.rect.top + v_offset + i*gap_y + j*self.objects[index].rect.height)))
